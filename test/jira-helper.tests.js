...
describe('All the JIRA REST calls are performed correctly', () => {
    ...
    it('a JIRA session header can be constructed', async () => {
        let authName = authHeaders.substring(0, authHeaders.indexOf('='));
        let authToken = authHeaders.substring(authHeaders.indexOf('=') + 1, authHeaders.length);
    ...
    it('a JIRA issue can be created', async () => {
        ...
        let response = await jira.createJiraIssue(authHeaders, JSON.stringify(mocks.MOCK_JIRA_ISSUE_CREATION_PAYLOAD));
    ...
    it('a list of JIRA issues can be queried', async () => {
        ...
        let response = await jira.searchExistingJiraIssues(authHeaders);
    ...
    it('a JIRA session can be invalidated', async () => {
        ...
        let response = await jira.invalidateJiraSession(authHeaders);
